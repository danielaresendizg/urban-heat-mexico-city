...
# Ruta de tu archivo
file_path = "/Users/danielaresendiz/Library/CloudStorage/OneDrive-UniversityCollegeLondon(2)/Dissertation/01_data/01_Manzana/manzanas_IVS_20250723.csv"
...
for thermal_var in thermal_vars:
    ...
    for social_var in social_vars:
        if social_var not in df.columns:
            ...
            continue
        try:
            df['grupo_social'] = pd.qcut(df[social_var], 3, labels=[1,2,3])
        except ValueError:
            print(f"Advertencia: no se pudo calcular terciles para {social_var} (valores repetidos o insuficientes)")
            continue

        groups = df['grupo_social'].dropna().unique()
        significant_counts = {g: 0 for g in groups}
        for g1, g2 in combinations(groups, 2):
            vals1 = df[df['grupo_social'] == g1][thermal_var].dropna()
            vals2 = df[df['grupo_social'] == g2][thermal_var].dropna()
...
heatmap_data.to_excel("tabla_heatmap_significancia_terciles.xlsx")
...
plt.title("Diferencias significativas entre terciles\npor indicador social y variable térmica", fontsize=17, pad=20)
plt.xticks(fontsize=12, rotation=30, ha='right')
plt.yticks(fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.98])
plt.savefig("heatmap_dif_signif_vulnerabilidad_terciles.png", dpi=400)
plt.show()

print("✅ ¡Listo! Tabla y heatmap generados con terciles.")
