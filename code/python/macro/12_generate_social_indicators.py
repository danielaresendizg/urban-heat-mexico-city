...
vars_needed = [
    ...
    "P3YM", "P3YM_HLI", "POB_AFRO", "PCON_DISC",
...
prop_vars = [
    ...
    # Ethnic
    ("pct_ethnic_afro", lambda df: np.where(df["P3YM"] == 0, 0, df["POB_AFRO"] / df["P3YM"] * 100)),
    ("pct_ethnic_ind",  lambda df: np.where(df["P3YM"] == 0, 0, df["P3YM_HLI"] / df["P3YM"] * 100)),
    ("pct_ethnic_other",lambda df: np.where(df["P3YM"] == 0, 0, (df["POBTOT"] - (df["POB_AFRO"] + df["P3YM_HLI"])) / df["P3YM"] * 100)),
    ...
    # Mobility
    ("pct_pop_auto",       lambda df: np.where(df["POBTOT"] == 0, 0, (df["PROM_OCUP"] - (df["VPH_NDACMM"] * df["PROM_OCUP"])) / df["POBTOT"] * 100)),
    ("pct_pop_sin_auto",   lambda df: np.where(df["POBTOT"] == 0, 0, (df["VPH_NDACMM"] * df["PROM_OCUP"]) / df["POBTOT"] * 100)),
...
final_cols = [c for c, _ in prop_vars] + ['CVEGEO']
